...
## Работа сервера
1. Создайте базу данных PersonDB. Выполните все SQL скрипты перед запуском программы (SQL commands) (проект работает через докерфайл)
   ```sql
   create table Users
   (
    Id       int identity
...
7. Пройдя верификацию вы попадёте на страницу "панель администратора".
   Здесь вы можете добавлять и удалять элементы таблиц 
